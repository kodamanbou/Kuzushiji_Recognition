...
def parse_anno(annotation_path, target_size=None):
    ...
    result = []
    for line in anno.values:
        s = str(line[1]).strip().split(' ')
        img = Image.open(img_path + str(line[0]) + '.jpg')
        print(line[0])
        img_w, img_h = img.width, img.height
        w_ratio = target_size[0] / img_w
        h_ratio = target_size[1] / img_h

        box_cnt = len(s) // 5
        for i in range(box_cnt):
            width = float(s[i * 5 + 3]) * w_ratio
            height = float(s[i * 5 + 4]) * h_ratio
            assert width > 0
            assert height > 0
...
if __name__ == '__main__':
    ...
    target_size = [416, 416]
    annotation_path = '../input/train.csv'
